...
impl Prf {
    ...
    //     HmacSha256::new_from_slice(passphrase.as_bytes()).expect("HMAC accepts all key sizes")
    // }

    // Calculate hashed message authentication code using SHA-512 digest
    // pub fn hmac512(&self, passphrase: &str) -> Hmac<Sha512> {
    //     type HmacSha512 = Hmac<Sha512>;
    //     HmacSha512::new_from_slice(passphrase.as_bytes()).expect("HMAC accepts all key sizes")
    // }
}
